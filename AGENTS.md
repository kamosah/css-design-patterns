...
Do not use agentic naming in branches, commits, PR titles, PR bodies, or commit trailers. Avoid names or metadata such as `codex/...`, `[codex] ...`, `Co-authored-by: Claude`, `Co-authored-by: Codex`, or `Co-authored-by: GitHub`. Branches, commits, and PR titles should use project-relevant names only, and commits should list only the current human author unless explicitly requested otherwise.
